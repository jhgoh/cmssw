...
TracktoRPC::TracktoRPC(const reco::TrackCollection * alltracks, const edm::EventSetup& iSetup, const edm::ParameterSet& iConfig, const edm::InputTag& tracklabel){
  ...
  std::set<uint32_t> rpcput;
  const double MaxD=999.;
      ...
      if ( id.subdetId() != MuonSubdetId::DT ) {
        ...
        for ( auto trajectory : trajectories ) {
          ...
          for ( auto iteraRoll : rollsForThisDT ) {
            ...
            if ( !ptss.isValid() or !ValidRPCSurface(rollasociated->id().rawId(), ptss.localPosition(), iSetup) ) continue;
    ...
    for( auto rpcroll2 : rpcrolls2 ) {
      ...
      if ( id.subdetId() == MuonSubdetId::DT) {
        ...
        for(auto trajectory : trajectories ) {
          ...
          if ( !ptss.isValid() or !ValidRPCSurface(rpcroll2, ptss.localPosition(), iSetup) ) continue;
